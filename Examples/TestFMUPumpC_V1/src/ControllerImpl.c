...
static fmiBoolean vrOutOfRange(fmiComponent c, const char* f,
		fmiValueReference vr, unsigned int end) {
	ModelInstance* comp = (ModelInstance*) c;
	if (vr >= end) {
		comp->functions.logger(comp, comp->instanceName, fmiError, "error",
...
fmiComponent fmiInstantiateSlave(fmiString instanceName, fmiString fmuGUID,
	...
	ModelInstance *c = (ModelInstance*) malloc(sizeof(ModelInstance));
